...
body.on('click','.main-nav__toggle',function(){$(this).toggleClass('main-nav__toggle--opened');$('.main-nav').toggleClass('main-nav--opened');$('.header__contacts').toggleClass('header__contacts--menu-active');$('.header').toggleClass('header--active');});$('.main-nav__list-item--hovered').swipe({tap:function(event,target){$(this).children('.main-nav__list-link').toggleClass('main-nav__list-link--active');}});$(".cb-testimonials__list-item").swipe({swipeLeft:function(event,direction,distance,duration,fingerCount,fingerData){next();},swipeRight:function(event,direction,distance,duration,fingerCount,fingerData){prev();},allowPageScroll:"vertical",threshold:25});function toggleFaq(isAccordion){body.on('click','.faqs__item-header',function(){$target=$(this);$item=$target.closest('.faqs__item');if(isAccordion){if($item.hasClass('faqs__item--active')){$item.removeClass('faqs__item--active');return;}
...
$('.get-renovation__btn').click(function(e){e.preventDefault();$('.popup').show();});$('.get-renovation__btn').keydown(function(e){if(e.which===ENTER_KEYCODE){e.preventDefault();$('.popup').show();}});$('.popup__close-btn').click(function(){$('.popup').hide();});$(document).keydown(function(e){if(e.which===ESC_KEYCODE){$('.popup').hide();}});$('.popup__btn').click(function(){$('.popup').hide();});$('.popup__btn').keydown(function(e){if(e.which===ENTER_KEYCODE){$('.popup').hide();}});$('.enahncify-way__show-more-btn').swipe({tap:function(event,target){$('.enahncify-way__content-block').addClass('enahncify-way__content-block--full');}});if($('.cb-testimonials__slides-list').length){var index,bullets,$slider,$sliderLength,$left_btn,$right_btn,$sliderWidth,$slideWidth;function initVars(){index=0;$bullets=$('.cb-testimonials__pointers-item');$slider=$('.cb-testimonials__slides-list');$sliderLength=$('.cb-testimonials__list-item').length;$left_btn=$('.cb-testimonials__list-controls--left');$right_btn=$('.cb-testimonials__list-controls--right');$sliderWidth=$('.cb-testimonials__slides-list').width();$slideWidth=$sliderWidth/$sliderLength;document.querySelector(".cb-testimonials__slides-list").style.transform="translateX(0)";};initVars();function checkBullets(){$bullets.removeClass('cb-testimonials__pointers-item--active');$($bullets[index]).addClass('cb-testimonials__pointers-item--active');};checkBullets();function checkUI(){if(index>0&&index<$sliderLength-1){$($left_btn).removeClass('cb-testimonials__list-controls--inactive');}
else if(index===0){$($left_btn).addClass('cb-testimonials__list-controls--inactive');$($right_btn).removeClass('cb-testimonials__list-controls--inactive');}
else if(index===$sliderLength-1){$($right_btn).addClass('cb-testimonials__list-controls--inactive');$($left_btn).removeClass('cb-testimonials__list-controls--inactive');}};function next(){checkUI();if(index===$sliderLength-1){var range=0;}else{var range=$slideWidth+$slideWidth*index;}
document.querySelector(".cb-testimonials__slides-list").style.transform="translateX(-"+range+"px)";if(index===$sliderLength-1){index=0;}else{index++;}
checkBullets();checkUI();};function prev(){if(index===0){var range=-$slideWidth*($sliderLength-1);}else{var range=($slideWidth-$slideWidth*index);}
document.querySelector(".cb-testimonials__slides-list").style.transform="translateX("+range+"px)";if(index===0){index=$sliderLength-1;}else{index--;}
checkBullets();checkUI();};body.on('click','.cb-testimonials__list-controls--left',function(){prev();});body.on('click','.cb-testimonials__list-controls--right',function(){next();});body.on('click','.cb-testimonials__pointers-item',function(evt){var id=$(this).data('id');document.querySelector(".cb-testimonials__slides-list").style.transform="translateX(-"+($slideWidth*(+id))+"px)";index=id;checkBullets();checkUI();});$(window).resize(throttle(initVars,500,{leading:false}))}